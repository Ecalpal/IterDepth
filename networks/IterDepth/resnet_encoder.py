...
class ResnetEncoderMatching(nn.Module):
    ...
    def forward(self, current_image, lookup_images, poses, K, invK, min_depth_bin=None, max_depth_bin=None):
        ...
        with torch.no_grad():
            ...
            cost_volume, missing_mask = self.match_features(current_feats, lookup_feats, poses, K, invK)
        ...
        cost_volume *= confidence_mask.unsqueeze(1) # [1,96,48,160]
        post_matching_feats = self.reduce_conv(torch.cat([self.features[-1], cost_volume], 1)) # [1,96,48,160] self.features[-1] [1,96,48,160]

        out = []

        out.append(self.layer2(post_matching_feats))
        out.append(self.layer3(out[-1]))
        out.append(self.layer4(out[-1]))

        return out, lowest_cost, confidence_mask
...
class ResnetEncoder_context(nn.Module):
    """Pytorch module for a resnet encoder
    """

    def __init__(self, num_layers, pretrained, num_input_images=1, **kwargs):
        super(ResnetEncoder_context, self).__init__()
    ...
    def forward(self, input_image):
        ...
        out.append(self.encoder.layer2(x))
        out.append(self.encoder.layer3(out[-1]))
        out.append(self.encoder.layer4(out[-1]))
        return out