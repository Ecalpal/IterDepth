...
class Trainer:
    def __init__(self, options):
        ...
        if self.train_teacher_and_pose:
            if not self.opt.fix_bins:
                ...
                print('using fixed depth binning!')
        else:
        ...
        self.models["encoder_context"] = networks.ResnetEncoder_context(self.opt.num_layers, self.opt.weights_init == "pretrained")
        self.models["encoder_context"].to(self.device)

        self.models["depth"] = networks.DepthDecoder_GRU(iters=self.opt.iters, res=self.opt.num_layers)
        self.models["depth"].to(self.device)

        # from thop import profile
        # input_t1 = torch.randn(1, 3, 192, 640).cuda()
        # flops1, para_encoder = profile(self.models["encoder_context"], inputs=(input_t1, ), verbose=False)
        # input_t2 = torch.randn(1, 128, 24, 80).cuda()
        # input_t3 = torch.randn(1, 128, 24, 80).cuda()
        # flops2, para_decoder = profile(self.models["depth"], inputs=(input_t2, input_t3, ), verbose=False)
        ...
        self.models["pose_encoder"] =  networks.ResnetEncoder(18, self.opt.weights_init == "pretrained", num_input_images=self.num_pose_frames)
        ...
        self.model_optimizer = optim.NAdam(self.parameters_to_train, self.opt.learning_rate)
    ...
    def set_train(self):
        ...
        for k, m in self.models.items():
            if self.train_teacher_and_pose:
                ...
                # if teacher + pose is frozen, then only use training batch norm stats for multi components
    ...
    def process_batch(self, inputs, is_train=False):
        ...
        self.generate_images_pred_for_mono_model(inputs, mono_outputs)
        mono_losses = self.compute_losses_for_mono_model(inputs, mono_outputs)
        ...
        # multi frame path
        encoder_output, lowest_cost, confidence_mask = self.models["encoder"](inputs["color_aug", 0, 0],
                                                                            lookup_frames,
                                                                            relative_poses,
                                                                            inputs[('K', 2)],
                                                                            inputs[('inv_K', 2)],
        ...
        outputs["lowest_cost"] = F.interpolate(lowest_cost.unsqueeze(1), [self.opt.height, self.opt.width], mode="nearest")[:, 0]
        outputs["consistency_mask"] = F.interpolate(confidence_mask.unsqueeze(1), [self.opt.height, self.opt.width], mode="nearest")[:, 0]

        if not self.opt.disable_motion_masking:
            outputs["consistency_mask"] = (outputs["consistency_mask"] * self.compute_matching_mask(outputs))
    ...
    def generate_images_pred_for_mono_model(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                outputs[("color", frame_id, scale)] = F.grid_sample(inputs[("color", frame_id, source_scale)], outputs[("sample", frame_id, scale)], padding_mode="border", align_corners=True)
                ...
                if self.opt.use_depth_hints:
                    if frame_id == 's' and scale == 0:
                        ...
                        outputs[("color_depth_hint_mono", frame_id, scale)] = F.grid_sample(inputs[("color", frame_id, source_scale)], pix_coords, padding_mode="border", align_corners=True)
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        Generated images are saved into the `outputs` dictionary.
        """
        for ite in range(self.opt.iters):
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                outputs[("color", frame_id, scale, ite)] = F.grid_sample(inputs[("color", frame_id, source_scale)], outputs[("sample", frame_id, scale, ite)], padding_mode="border", align_corners=True)

                if not self.opt.disable_automasking:
                    outputs[("color_identity", frame_id, scale, ite)] = inputs[("color", frame_id, source_scale)]

                if self.opt.use_depth_hints:
                    if frame_id == 's' and ite == self.opt.iters-1:
                        # generate depth hint warped image (only max scale and for stereo image)
                        depth = inputs['depth_hint']
                        cam_points = self.backproject_depth[source_scale](depth, inputs[("inv_K", source_scale)])
                        pix_coords = self.project_3d[source_scale](cam_points, inputs[("K", source_scale)], T)

                        outputs[("color_depth_hint", frame_id, ite)] = F.grid_sample(inputs[("color", frame_id, source_scale)], pix_coords, padding_mode="border", align_corners=True)
    ...
    def compute_losses_for_mono_model(self, inputs, outputs):
        ...
        if self.opt.use_depth_hints:
            pred = outputs[("color_depth_hint_mono", 's', 0)]
        ...
        for scale in self.opt.scales:
            ...
            outputs["identity_selection_mono/{}".format(scale)] = (1 - reprojection_loss_mask).float()
            losses['reproj_loss_mono/{}'.format(scale)] = reprojection_loss
            ...
            if self.opt.use_depth_hints:
                ...
                # save for logging
                outputs["depth_hint_pixels_mono/{}".format(scale)] = depth_hint_loss_mask
                losses['depth_hint_loss_mono/{}'.format(scale)] = depth_hint_loss
            ...
            total_loss += loss
            losses["loss_mono/{}".format(scale)] = loss

        total_loss /= self.num_scales
        losses["loss_mono"] = total_loss
    ...
    def compute_losses(self, inputs, outputs):
        ...
        # compute depth hint reprojection loss
        if self.opt.use_depth_hints:
            pred = outputs[("color_depth_hint", 's', self.opt.iters-1)]
            depth_hint_reproj_loss = self.compute_reprojection_loss(pred, inputs[("color", 0, 0)])
            # set loss for missing pixels to be high so they are never chosen as minimum
            depth_hint_reproj_loss += 1000 * (1 - inputs['depth_hint_mask'])
        else:
        ...
        for ite in range(self.opt.iters):
            ...
            i_weight = 0.8**(self.opt.iters - ite - 1)
            ...
            if not self.opt.disable_automasking:
                ...
                for frame_id in self.opt.frame_ids[1:]:
                    pred = inputs[("color", frame_id, source_scale)]
                    identity_reprojection_losses.append(self.compute_reprojection_loss(pred, target)*i_weight)
            ...
            # find which pixels to apply reprojection loss to, and which pixels to apply consistency loss to
            ...
            if not self.opt.no_matching_augmentation:
                reprojection_loss_mask = (reprojection_loss_mask * (1 - outputs['augmentation_mask']))
            ...
            multi_depth = outputs[("depth", 0, ite)]
            ...
            consistency_target = (mono_depth.detach() * consistency_mask + multi_depth.detach() * (1 - consistency_mask))
            ...
            if self.opt.use_depth_hints:
                ...
                depth_hint_loss = depth_hint_loss.sum() / (depth_hint_loss_mask.sum() + 1e-7)
                # save for logging
                outputs["depth_hint_pixels/{}".format(ite)] = depth_hint_loss_mask
                losses['depth_hint_loss/{}'.format(ite)] = depth_hint_loss

            loss += reprojection_loss + consistency_loss + depth_hint_loss
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            ...
            for frame_id in self.opt.frame_ids:
                ...
                if s == 0 and frame_id != 0:
                    writer.add_image(f"color_pred_{frame_id}_{s}/{j}", outputs[("color", frame_id, s)][j].data, self.step)
            ...
            if outputs.get("lowest_cost") is not None:
                ...
                consistency_mask = outputs['consistency_mask'][j].cpu().detach().unsqueeze(0).numpy()
                ...
                writer.add_image("lowest_cost_masked/{}".format(j), lowest_cost * consistency_mask, self.step)
                writer.add_image("consistency_mask/{}".format(j), consistency_mask, self.step)

                consistency_target = colormap(outputs["consistency_target/0"][j])
                writer.add_image("consistency_target/{}".format(j), consistency_target, self.step)
    ...
    def save_model(self, save_step=False):
        ...
        for model_name, model in self.models.items():
            ...
            if model_name == 'encoder':
                ...
                if self.opt.fix_bins:
                    to_save['min_depth_bin'] = self.opt.min_depth
                    to_save['max_depth_bin'] = self.opt.max_depth
                else:
